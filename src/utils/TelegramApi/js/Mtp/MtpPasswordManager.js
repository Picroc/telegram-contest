...
export default class MtpPasswordManagerModule {
    ...
    getState = (options) => this.MtpApiManager.mtpInvokeApi('account.getPassword', {}, options).then((result) => {
        console.log(result);
        return result;
    });
    ...
    updateSettings = (state, settings) => {
        ...
        return Promise.all([currentHashPromise, newHashPromise]).then((hashes) => {
            ...
            return this.MtpApiManager.mtpInvokeApi('account.updatePasswordSettings', params);
        });
    };

    check = (state, password, options) => this.makePasswordHash(state.current_salt, password).then((passwordHash) => {
        return this.MtpApiManager.mtpInvokeApi('auth.checkPassword', {
            password_hash: passwordHash
        }, options);
    });

    requestRecovery = (state, options) => this.MtpApiManager.mtpInvokeApi('auth.requestPasswordRecovery', {}, options);

    recover = (code, options) => this.MtpApiManager.mtpInvokeApi('auth.recoverPassword', {
        code: code
    }, options);
