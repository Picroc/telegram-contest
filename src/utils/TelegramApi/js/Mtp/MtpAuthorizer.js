...
export default class MtpAuthorizerModule {
    ...
    mtpSendReqPQ = (auth) => new Promise((resolve, reject) => {
        const request = new TLSerialization({ mtproto: true });

        request.storeMethod('req_pq', { nonce: auth.nonce });

        console.log(dT(), 'Send req_pq', bytesToHex(auth.nonce));
        this.mtpSendPlainRequest(auth.dcID, request.getBuffer()).then((deserializer) => {
    ...
    mtpSendSetClientDhParams = (auth) => new Promise((resolve, reject) => {
        ...
        this.CryptoWorker.modPow(gBytes, auth.b, auth.dhPrime).then((gB) => {
            ...
            this.mtpSendPlainRequest(auth.dcID, request.getBuffer()).then((deserializer) => {
                ...
                this.CryptoWorker.modPow(auth.gA, auth.b, auth.dhPrime).then((authKey) => {
                    ...
                    switch (response._) {
                        case 'dh_gen_ok':
                            newNonceHash1 = sha1BytesSync(auth.newNonce.concat([1], authKeyAux)).slice(-16);
                            ...
                            serverSalt = bytesXor(auth.newNonce.slice(0, 8), auth.serverNonce.slice(0, 8));
                        ...
                        case 'dh_gen_retry':
                            newNonceHash2 = sha1BytesSync(auth.newNonce.concat([2], authKeyAux)).slice(-16);
                        ...
                        case 'dh_gen_fail':
                            newNonceHash3 = sha1BytesSync(auth.newNonce.concat([3], authKeyAux)).slice(-16);
        ...
        return new Promise((resolve, reject) => {
            $timeout(() => {
                ...
                .then((result) => {
                    ...
                    console.log('DATA', this.cached[dcID]);
